...
if(is_page_template( 'template-blog.php' )){
    if(teamhost_get_theme_mod('blog_template_sidebar_custom',true) == 'custom'){
        $sidebar_position= teamhost_get_theme_mod('blog_template_sidebar_position',true);
        $sticky_sidebar = teamhost_get_theme_mod('blog_template_sidebar_sticky',true);
    }
}

if($sidebar_position == 'left'){
    $page_sidebar_position = 'sidebar_left';
}else if($sidebar_position== 'right') {
    $page_sidebar_position = 'left_sidebar_right';
}
...
<?php if( is_active_sidebar( 'sidebar-stream-left' )) { ?>
<div class="uk-width-1-4@l uk-width-2-5@m uk-width-2-5@s sidebar_left <?php echo esc_attr($page_sidebar_position); ?> <?php echo esc_attr($page_sidebar_sticky); ?>">
    <aside class="l-sidebar">
        <?php dynamic_sidebar( 'sidebar-stream-left' ); ?>
    </aside>
</div>
<?php } ?>
