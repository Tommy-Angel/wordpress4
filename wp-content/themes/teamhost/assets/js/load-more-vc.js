jQuery.noConflict()(function($) {
    ...
    var initLoadMoreFunctionVc = function() {
        var body = $('body');
        var page_num = parseInt(teamhost_vc_pagination_data.startPage) + 1;
        var max_pages = parseInt(teamhost_vc_pagination_data.maxPages);
        var next_link = teamhost_vc_pagination_data.nextLink;

        var container = teamhost_vc_pagination_data.container;
        var $container = $(container);
        var container_has_isotope = false;


        var last_btn_text = teamhost_vc_pagination_data.button_text_no_post;
            ...
            if (!$(this).hasClass('loading') && !$(this).hasClass('load_more_disable') && page_num <= max_pages) {
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        $button.text(teamhost_vc_pagination_data.button_loading);
                    },
                    complete: function(content) {
                        $button.text(teamhost_vc_pagination_data.button_text);
