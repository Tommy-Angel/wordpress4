...
<form action="<?php bp_activity_post_form_action(); ?>" method="post" id="youzify-wall-form" class="youzify-wall-form" name="whats-new-form" enctype="multipart/form-data">
	...
	<div id="whats-new-content" class="youzify-wall-content">
		...
		<div class="youzify-wall-author" href="<?php echo bp_loggedin_user_domain(); ?>"><?php bp_loggedin_user_avatar(array( 'type' => 'full' )); ?></div>

			<textarea name="status" class="youzify-wall-textarea bp-suggestions" id="whats-new" placeholder="<?php if ( bp_is_group() )
		printf( __( "What's new in %s, %s?", 'teamhost' ), bp_get_group_name(), bp_get_user_firstname( bp_get_loggedin_user_fullname() ) );
	else
		printf( __( "What's new, %s?", 'teamhost' ), bp_get_user_firstname( bp_get_loggedin_user_fullname() ) );
	?>" <?php if ( bp_is_group() ) : ?> data-suggestions-group-id="<?php echo esc_attr( (int) bp_get_current_group_id() ); ?>" <?php endif; ?>
			...
			<div class="youzify-lp-prepost" data-loaded="false">
			    ...
			    <div class="lp-prepost-wrap">
			        ...
			        <div class="clearfix lp-preview-pagination">
			            ...
			            <span class="lp-preview-thubmnail-pagination">{{thumbnailPaginationText}}</span><span class="lp-pagination-of"><?php _e( 'of', 'teamhost' ); ?></span><span class="lp-preview-thubmnail-text">{{thumbnailText}}</span>
			        ...
			        <div class="lp-preview-no-thubmnail">
			            <label class="lp-preview-no-thubmnail-text">
			                ...
			                <span><?php _e( 'No thumbnail', 'teamhost' ) ?></span>
			            </label>
			        </div>
		...
		<div class="youzify-wall-custom-form youzify-wall-link-form" data-post-type="activity_link">
			...
			<div class="youzify-wall-cf-item">
				<input type="text" class="youzify-wall-cf-input" name="link_url" placeholder="<?php _e( 'Add Link URL', 'teamhost' ); ?>">
			</div>

			<div class="youzify-wall-cf-item">
				<input type="text" class="youzify-wall-cf-input" name="link_title" placeholder="<?php _e( 'Add Link Title', 'teamhost' ); ?>">
			</div>

			<div class="youzify-wall-cf-item">
				<textarea name="link_desc" class="youzify-wall-cf-input" placeholder="<?php _e( 'Brief Link Description', 'teamhost' ); ?>"></textarea>
			</div>
		...
		<div class="youzify-wall-custom-form youzify-wall-quote-form" data-post-type="activity_quote">
			...
			<div class="youzify-wall-cf-item">
				<input type="text" class="youzify-wall-cf-input" name="quote_owner" placeholder="<?php _e( 'Add Quote Owner', 'teamhost' ); ?>">
			</div>

			<div class="youzify-wall-cf-item">
				<textarea name="quote_text" class="youzify-wall-cf-input" placeholder="<?php _e( 'Add Quote Text', 'teamhost' ); ?>"></textarea>
			</div>
		...
		<div class="youzify-wall-custom-form youzify-wall-giphy-form" data-post-type="activity_giphy">
			...
			<div class="youzify-wall-cf-item">
				<div class="youzify-giphy-search-form">
					<input type="text" class="youzify-wall-cf-input youzify-giphy-search-input" name="giphy_search" placeholder="<?php _e( 'Search for GIFs...', 'teamhost' ); ?>">
				</div>
				...
				<div class="youzify-giphy-items-content">
					...
					<div class="youzify-no-gifs-found"><i class="far fa-frown"></i><?php _e( 'No GIFs found', 'teamhost' ); ?></div>
				</div>
			</div>
	...
	<div class="youzify-wall-actions" id="youzify-wall-actions">
		...
		<div class="youzify-form-tools">
			...
			<?php if ( apply_filters( 'youzify_allow_wall_upload_attachments', true ) ) : ?>
				<div class="youzify-wall-upload-btn youzify-form-tool" data-youzify-tooltip="<?php _e( 'Upload Attachment', 'teamhost' ); ?>"><i class="fas fa-paperclip"></i></div>
			<?php endif; ?>
		...
		<div class="youzify-posting-form-actions">
			...
			<?php if ( bp_is_active( 'groups' ) && ! bp_is_my_profile() && ! bp_is_group() ) : ?>
				...
				<div id="whats-new-post-in-box">
					...
					<label for="whats-new-post-in" ><?php _e( 'Post in:', 'teamhost' ); ?></label>
					<select id="whats-new-post-in" name="whats-new-post-in">
						<option selected="selected" value="0"><?php _e( 'My Profile', 'teamhost' ); ?></option>
			...
			<button type="submit" name="aw-whats-new-submit" class="youzify-wall-post"><?php esc_attr_e( 'Post', 'teamhost' ); ?></button>
