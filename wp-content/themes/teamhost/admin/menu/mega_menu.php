<?php
if ( ! function_exists( 'teamhost_fontawesome_menu_icons' ) ) :
    function teamhost_fontawesome_menu_icons() {
        $menu_font = array("disable",
            ...
            "fa fa-calendar-check-o","fa fa-calendar-minus-o","fa fa-calendar-o","fa fa-calendar-plus-o","fa fa-calendar-times-o","fa fa-camera","fa fa-camera-retro","fa fa-transport","fa fa-caret-down","fa fa-caret-left","fa fa-caret-right",
...
if ( ! function_exists( 'teamhost_menu_icons' ) ) :
    function teamhost_menu_icons() {
        $menu_font = array(
            "disable",
...
if ( ! function_exists( 'teamhost_contact_forms' ) ) :
    function teamhost_contact_forms() {

        $contact_forms_fields = get_posts(array('post_type' => 'wpcf7_contact_form', 'fields' => 'ids'));

        $contact_forms = array('disable' => 'Disable');

        foreach ($contact_forms_fields as $cf){
            $contact_forms[$cf] = get_the_title($cf);
        }
...
if (!class_exists('teamhost_Mega_menu')) {
	class teamhost_Mega_menu {
		...
		public static function options() {
           // $icon_ListMenu = teamhost_fontawesome_menu_icons();
            $icon_ListMenu = teamhost_menu_icons();
            $contact_forms = teamhost_contact_forms();

			return array(
			    '_teamhost_mega_menu_icon'	=> array(
                    'type'      => 'select_icon',
                    'label'     => esc_html__('Select Icon', 'teamhost'),
                    'default'   => 0,
                    'options'   => $icon_ListMenu,
                    'size'      => 'wide',
                ),
				'_teamhost_mega_menu_image'	=> array(
						'type' => 'upload',
						'label' => esc_html__('Image', 'teamhost'),
						'default' => '',
						'size' => 'wide',
						'depth' => '0',  
						'class' => 'teamhost-show-only-depth-0',
					),
                '_teamhost_mega_menu_modal_form'	=> array(
                    'type' => 'select',
                    'label' => esc_html__( 'Modal Contact 7 Form Select', 'teamhost' ),
                    'default' => 0,
				...
				'_teamhost_mega_menu_bg_position'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Background position', 'teamhost' ),
						'default' => 0,
						'options' => array(
								'left top'              => esc_html__('Left top', 'teamhost'),
								'left center'           => esc_html__('Left center', 'teamhost'),
								'left bottom'           => esc_html__('Left bottom', 'teamhost'),
								'right top'             => esc_html__('Right top', 'teamhost'),
								'right center'          => esc_html__('Right center', 'teamhost'),
								'right bottom'          => esc_html__('Right bottom', 'teamhost'),
								'center top'            => esc_html__('Center top', 'teamhost'),
								'center center'         => esc_html__('Center center', 'teamhost'),
								'center bottom'         => esc_html__('Center bottom', 'teamhost')
							),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'teamhost-show-only-depth-0',
					),
				'_teamhost_mega_menu_bg_repeat'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Background repeat', 'teamhost' ),
						'default' => 'no-repeat',
						'options' => array(
								'no-repeat'         => esc_html__( 'No-repeat', 'teamhost' ),
								'repeat'            => esc_html__( 'Repeat', 'teamhost' ),
								'repeat-x'          => esc_html__( 'Repeat-x', 'teamhost' ),
								'repeat-y'          => esc_html__( 'Repeat-y', 'teamhost' ),
							),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'teamhost-show-only-depth-0',
				...
				'_teamhost_mega_menu_limit_columns'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Limit Max columns number', 'teamhost' ),
						'default' => 0,
						'options' => array(
								''	=>	esc_html__( 'None', 'teamhost' ),
								1	=>	esc_html__( 'One columns', 'teamhost' ),
								2	=>	esc_html__( 'Two columns', 'teamhost' ),
								3	=>	esc_html__( 'Three columns', 'teamhost' ),
                                4	=>	esc_html__( 'Four columns', 'teamhost' ),
                                5	=>	esc_html__( 'Five columns', 'teamhost' ),
							),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'teamhost-show-only-depth-0 teamhost-columns-limit',
					),
			);
		}
		...
		private function _add_filters() {
			...
			add_action('admin_enqueue_scripts', array( $this, 'teamhost_mega_menu_admin_scripts' ), 80);
		}
		
		
 
		function teamhost_mega_menu_admin_scripts() {
			wp_enqueue_media();
		}
		...
		public function apply_edit_walker_class( $walker, $menu_id ) {
			return teamhost_EDIT_MENU_WALKER_CLASS;
		}
	}
}
