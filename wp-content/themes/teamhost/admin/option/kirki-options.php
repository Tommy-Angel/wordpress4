<?php
if (!function_exists('teamhost_customize_register'))
{
    function teamhost_customize_register($wp_customize)
    {
...
add_action('customize_register', 'teamhost_customize_register');

if (!function_exists('initial_kirki_options') && class_exists('teamhost_Options'))
{

    function initial_kirki_options()
    {
    teamhost_Options::add_config(array(
    ...
    if ( class_exists('BuddyPress') && class_exists('Youzify')) {
        //Community Setting
        get_template_part('admin/option/kirki_option/community/community_option');
    }

    add_action('init', 'initial_kirki_options');
}
if (!function_exists('teamhost_get_theme_mod')) {
    function teamhost_get_theme_mod($name = null, $use_acf = null, $postId = null, $acf_name = null)
    {
        $value = null;
        ...
        if ($use_acf) {
            $value = teamhost_get_metabox($acf_name ? $acf_name : $name, $postId);
        }

        // get value from options
        if (($value === null || $value === 'default')) {
            if (class_exists('teamhost_Options')) {
                $value = teamhost_Options::get_option($name);
            }
        }

        $value = apply_filters('teamhost_filter_get_theme_mod', $value, $name);
        return $value;
    }
}
// get metabox
if (!function_exists( 'teamhost_get_metabox' )):
    function teamhost_get_metabox($name = null, $postId = null)
    {
        $value = null;
