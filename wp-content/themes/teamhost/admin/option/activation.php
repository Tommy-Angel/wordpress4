...
function teamhost_check_is_activated(){
	return true;
    $envatoCode = get_theme_mod('teamhost_licence_settings_code', '');
...
function teamhost_theme_code_info()
{
    return ['theme_id' => '42511674', 'theme' => 'Theme Teamhost', 'envato_code' => get_theme_mod('teamhost_licence_settings_code', '') ];
}
function activate_license($envatoCode, $option_name, $option_name_code) {

    $theme_id = '42511674';
    $theme = 'Theme teamhost';
    ...
    if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
        ...
        if ( is_wp_error( $response ) ) {
            ...
            $message = __( 'An error occurred, please try again.', 'teamhost' );
        }
...
if ( !function_exists( 'teamhost_admin_notice_activation' ) ) {
    function teamhost_admin_notice_activation() {
        if(teamhost_check_is_activated())return;
        ...
        if ( $screen->id != 'appearance_page_adminpanel' ) {
            if ( 1 ) {
                ...
                <div class='update-nag'>
                    <h3 class='pix_notice_title'>" . esc_html__("Theme activation", "teamhost") . "</h3>
                    <p>" . esc_html__("Your code", "teamhost") . "<input type='text' name='pix_code' id='pix_code_activation_input' data-activationtheme='1' value='{$envatoCode}'><a data-activationtheme='1' class='button button-primary activation-theme'>" . esc_html__('Activation', 'teamhost') . "</a></p><p class='activated'>" . esc_html__("Theme activated", "teamhost") . "</p>
                </div>
            ";
            }
        }
...
function teamhost_admin_notice_view() {
    if (current_user_can('switch_themes')) {
        add_action('admin_notices', 'teamhost_admin_notice_activation', 2);
    }
}

if( wp_doing_ajax() ){
    add_action('wp_ajax_theme_activation', 'teamhost_ajax_theme_activation');
    add_action('wp_ajax_delete_key_activation', 'teamhost_ajax_delete_key_activation');
}
function teamhost_ajax_theme_activation() {
    global $post;
    if( ! wp_verify_nonce( $_POST['nonce_code'], 'pix-admin-nonce' ) ) die( 'Stop!');
    $json = wp_unslash( $_POST['code']);
    set_theme_mod( 'teamhost_licence_settings_code', $json );

    if($rez = teamhost_check_is_activated()){
        echo ''.$rez;
    }else{
        echo ''.$rez;
    }
...
function teamhost_ajax_delete_key_activation() {
    global $post;
    if( ! wp_verify_nonce( $_POST['nonce_code'], 'pix-admin-nonce' ) ) die( 'Stop!');
    ...
    if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
        ...
        if (is_object($license_data) && true == $license_data->success ) {
            set_theme_mod( 'teamhost_licence_settings_code', '' );
            update_option( 'teamhost_licence_code', '' );
            echo  1 ;
        }else{
            echo  0;
        }
