...
//						'url'           => $this->upload_url . $fonts_info['<css>'],
	...
	public function setup_menu() {
		add_theme_page( __( 'Font Icons Loader', 'tm-fonts' ), __( 'Font Icons Loader', 'tm-fonts' ), 'manage_options', $this->_token, [
			$this,
			'upload_page'
		] );
	}
	...
	private function _remove( $font_name, $need_make_css = true ) {
		if ( ! empty( $font_name ) ) {
			...
			if ( ! empty( $fonts_info['fonts'][ $font_name ] ) ) {
				...
				return sprintf( __( 'Font %s was successfully removed', 'tm-fonts' ), $font_name );
			}
	...
	private function _add( $movefile, $version = '' ) {
		...
		if ( $movefile && empty( $movefile['error'] ) ) {
			if ( $movefile['type'] != 'application/zip' ) {
				return new WP_Error( 'tm_upload_file_error', __( 'File is not Zip-archive.', 'tm-fonts' ) );
			}
			...
			if ( ! $result ) {
				...
				return new WP_Error( 'tm_unzip_file_error', __( 'Unzip file error.', 'tm-fonts' ) );
			}
			...
			if ( ! $font_dir ) {
				...
				return new WP_Error( 'tm_unzip_file_error', __( 'Can`t recognize SVG font file', 'tm-fonts' ) );
			}
			...
			if ( ! $this->is_dir( $font_dir ) ) {
				...
				return new WP_Error( 'tm_unzip_file_error', sprintf( __( 'Font %s is already installed, remove it before uploading.', 'tm-fonts' ), $font_name ) );
			}
			...
			foreach ( $files AS $file ) {
				if ( preg_match( '~\.(eot|svg|ttf|woff)$~i', $file, $match ) ) {
					...
					if ( ! $result ) {
						return new WP_Error( 'tm_unzip_file_error', __( 'Can`t copy font file', 'tm-fonts' ) );
					}
...
"; //todo-me сделать минимизацию
			...
			return __( 'Font was successfully uploaded', 'tm-fonts' );
		} else {
	...
	private function _action() {
		...
		if ( ! empty( $_POST['remove'] ) ) {
			...
			if ( ! isset( $_FILES['tm_upload_icons'] ) ) {
				return new WP_Error( 'tm_upload_file_empty', __( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.', 'tm-fonts' ) );
			}
	...
	public function upload_page() {
		...
		<div class="wrap tm-fonts__page">
			<h1><?php esc_html_e( 'Font Icons Loader', 'tm-fonts' ) ?></h1>
			<div class="tm-fonts__container">
					...
					foreach ( $errors AS $error ) {
						...
						<div class="notice notice-error is-dismissible">
							...
							<button type="button" class="notice-dismiss">
								<span
									class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'tm-fonts' ); ?></span>
							</button>
						</div>
						<?php
					}
				...
				<div class="tm-fonts__upload-form">
					<h3><?php esc_html_e( 'Upload a Zip with font files', 'tm-fonts' ) ?></h3>
				...
				<div class="tm-fonts__header-wrap">
					<h3><?php esc_html_e( 'Installed fonts', 'tm-fonts' ) ?></h3>
					<div>
						<button type="button"
								class="tm-fonts__expand-all"><?php esc_html_e( 'Expand all', 'tm-fonts' ) ?></button>
						/
						<button type="button"
								class="tm-fonts__collapse-all"><?php esc_html_e( 'Collapse all', 'tm-fonts' ) ?></button>
					</div>
				</div>
				<form method="post">
						...
						<?php foreach ( $fonts_info['fonts'] as $font_name => $font ) { ?>
							<li class="tm-fonts__item">
								...
								<div class="tm-fonts__item-header">
									...
									<span
										class="tm-fonts__item-count">(<?php echo sprintf( esc_html__( 'icons: %s', 'tm-fonts' ), count( $font['unicodes'] ) ) ?>)</span>
									<button type="button"
			...
			<div class="tm-fonts__popup">
				<div class="tm-fonts__popup-container">
					...
					<div class="tm-fonts__popup-title"><?php esc_html_e( 'HTML', 'tm-fonts' ) ?></div>
					<div
						class="tm-fonts__popup-code"><?php echo esc_html( '<i class="' ) . '<span class="tm-fonts__code"></span>' . esc_html( '"><!-- --></i>' ); ?></div>
					<div
						class="tm-fonts__popup-title"><?php esc_html_e( 'Shortcode', 'tm-fonts' ) ?></div>
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'tm-fonts' ), $this->_version );
	} // End __clone ()
