...
define( 'TM_THEME_FONTS_IS_AJAX', function_exists( 'wp_doing_ajax' ) ? wp_doing_ajax() : ( is_admin() && defined( 'DOING_AJAX' ) && DOING_AJAX ) );
define( 'TM_THEME_FONTS_IS_AJAX_HEARTBEAT', TM_THEME_FONTS_IS_AJAX && ! empty( $_POST['action'] ) && ( $_POST['action'] == 'heartbeat' ) );
...
class TM_Fonts {
	...
	public function __construct( $file = '' ) {
		...
		if ( TM_THEME_FONTS_IS_AJAX_HEARTBEAT ) {
			return;
		}

		$this->_token   = 'tmc_fonts';
		...
		if ( is_admin() ) {
			...
			add_action( 'after_setup_theme', [ $this, 'need_install_font' ], 100 );
	...
	public function need_install_font() {
		$theme_fonts = apply_filters( 'tm_fonts_theme_font', [] );
		if ( ! empty( $theme_fonts ) && is_array( $theme_fonts ) ) {
			$fonts_info = get_option( 'tm_fonts_info' );
	...
	public function check_font_path() {
		...
		if ( empty( $fonts_info['<ver>'] ) ) {
			...
			update_option( 'tm_fonts_info', $fonts_info );
		}
		if ( ! empty( $fonts_info['<css>'] ) && ! $this->is_file( $this->upload_dir . $fonts_info['<css>'] ) ) {
			$this->make_css();
		}
		if ( ! empty( $fonts_info['<style>'] ) ) {
			unset( $fonts_info['<style>'] );
			if ( isset( $fonts_info['<path>'] ) ) {
				if ( $this->is_file( $fonts_info['<path>'] ) ) {
					$this->delete( $fonts_info['<path>'] );
				}
				unset( $fonts_info['<path>'] );
			}
			if ( isset( $fonts_info['<json>'] ) ) {
				unset( $fonts_info['<json>'] );
			}
			...
			if ( ! empty( $theme_fonts ) && is_array( $theme_fonts ) ) {
				foreach ( $theme_fonts AS $font_name => $font ) {
					...
					if ( array_key_exists( $font_name, $fonts_info['fonts'] ) ) {
						$this->_remove( $font_name, false );
						unset( $fonts_info['fonts'][ $font_name ] );
					}
				}
			}
...
//						'url'           => $this->upload_url . $fonts_info['<css>'],
	...
	public function setup_menu() {
	    /*
		add_theme_page( __( 'Font Icons Loader', 'tm-helper-core' ), __( 'Font Icons Loader', 'tm-helper-core' ), 'manage_options', $this->_token, [
			$this,
			'upload_page'
		] );
        */
        ...
        add_submenu_page('tmc-theme-dashboard',
            esc_html__('Font Icons Loader', 'tm-helper-core'),
            esc_html__('Font Icons Loader', 'tm-helper-core'),
            'manage_options',
            $this->_token,
            [ $this, 'upload_page' ]
	...
	private function _remove( $font_name, $need_make_css = true ) {
		if ( ! empty( $font_name ) ) {
			...
			if ( ! empty( $fonts_info['fonts'][ $font_name ] ) ) {
				...
				return sprintf( __( 'Font %s was successfully removed', 'tm-helper-core' ), $font_name );
			}
	...
	private function _add( $movefile, $version = '' ) {
		...
		if ( $movefile && empty( $movefile['error'] ) ) {
			if ( $movefile['type'] != 'application/zip' ) {
				return new WP_Error( 'tm_upload_file_error', __( 'File is not Zip-archive.', 'tm-helper-core' ) );
			}
			...
			if ( ! $result ) {
				...
				return new WP_Error( 'tm_unzip_file_error', __( 'Unzip file error.', 'tm-helper-core' ) );
			}
			...
			if ( ! $font_dir ) {
				...
				return new WP_Error( 'tm_unzip_file_error', __( 'Can`t recognize SVG font file', 'tm-helper-core' ) );
			}
			...
			if ( ! $this->is_dir( $font_dir ) ) {
				...
				return new WP_Error( 'tm_unzip_file_error', sprintf( __( 'Font %s is already installed, remove it before uploading.', 'tm-helper-core' ), $font_name ) );
			}
			...
			foreach ( $files AS $file ) {
				if ( preg_match( '~\.(eot|svg|ttf|woff)$~i', $file, $match ) ) {
					...
					if ( ! $result ) {
						return new WP_Error( 'tm_unzip_file_error', __( 'Can`t copy font file', 'tm-helper-core' ) );
					}
					$font_urls[ $match[1] ] = $font_url . basename( $file );
...
"; //todo-me сделать минимизацию
			...
			return __( 'Font was successfully uploaded', 'tm-helper-core' );
		} else {
			return new WP_Error( 'tm_upload_file_error', $movefile['error'] );
		}
	}

	private function _action() {

		if ( get_current_screen()->id != 'appearance_page_tm_fonts' ) {
			return '';
		}
		...
		if ( ! empty( $_POST['remove'] ) ) {
			...
			check_admin_referer( 'tm_upload_action', 'tm_upload_icons' );

			if ( ! isset( $_FILES['tm_upload_icons'] ) ) {
				return new WP_Error( 'tm_upload_file_empty', __( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.', 'tm-helper-core' ) );
			}

			$movefile = wp_handle_upload( $_FILES['tm_upload_icons'], [ 'test_form' => false ] );
	...
	public function upload_page() {
		...
		<div class="wrap tm-fonts__page">
			<h1><?php esc_html_e( 'Font Icons Loader', 'tm-helper-core' ) ?></h1>
			<div class="tm-fonts__container">
				<?php
					...
					foreach ( $errors AS $error ) {
						...
						<div class="notice notice-error is-dismissible">
							...
							<button type="button" class="notice-dismiss">
								<span
									class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'tm-helper-core' ); ?></span>
							</button>
						</div>
						<?php
					}
				...
				?>
				<div class="tm-fonts__upload-form">
					<h3><?php esc_html_e( 'Upload a Zip with font files', 'tm-helper-core' ) ?></h3>
					<form method="post" enctype="multipart/form-data">
						<?php wp_nonce_field( 'tm_upload_action', 'tm_upload_icons' ); ?>
						<input type='file' id='tm_upload_icons' name='tm_upload_icons'/>
						<?php submit_button( 'Upload' ) ?>
					</form>
				</div>
				<div class="tm-fonts__header-wrap">
					<h3><?php esc_html_e( 'Installed fonts', 'tm-helper-core' ) ?></h3>
					<div>
						<button type="button"
								class="tm-fonts__expand-all"><?php esc_html_e( 'Expand all', 'tm-helper-core' ) ?></button>
						/
						<button type="button"
								class="tm-fonts__collapse-all"><?php esc_html_e( 'Collapse all', 'tm-helper-core' ) ?></button>
					</div>
				</div>
				<form method="post">
					<ul class="tm-fonts__list">
						<?php foreach ( $fonts_info['fonts'] as $font_name => $font ) { ?>
							<li class="tm-fonts__item">

								<div class="tm-fonts__item-header">
									<?php if ( empty( $font['version'] ) ) { ?>
										<button type="submit" name="remove"
												value="<?php echo esc_attr( $font_name ); ?>"
												class="dashicons dashicons-no-alt tm-fonts__item-remove"></button>
									<?php } ?>
									<a href="#"
									   class="tm-fonts__icon-link js-tm-fonts-font"><?php echo esc_html( $font_name ); ?></a>
									<span
										class="tm-fonts__item-count">(<?php echo sprintf( esc_html__( 'icons: %s', 'tm-helper-core' ), count( $font['unicodes'] ) ) ?>)</span>
									<button type="button"
											class="dashicons dashicons-arrow-down-alt2 tm-fonts__item-view"></button>
								</div>
								<div class="tm-fonts__wrap">
									<ul class="tm-fonts__icon-list">
										<?php foreach ( $font['unicodes'] as $class_name => $code ) { ?>
											<li class="tm-fonts__icon">
												<a href="#" class="tm-fonts__icon-link js-tm-fonts-icon"
												   data-code="<?php echo esc_attr( $code ); ?>"
												   data-class="<?php echo esc_attr( $class_name ); ?>">
													<div class="tm-fonts__icon-wrap">
														<i class="<?php echo esc_attr( $class_name ); ?> tm-fonts__icon-img"></i>
														<span class="tm-fonts__icon-title">
			...
			<div class="tm-fonts__popup">
				<div class="tm-fonts__popup-container">
					<i class="tm-fonts__popup-img"></i>
					<div class="tm-fonts__popup-title"><?php esc_html_e( 'HTML', 'tm-helper-core' ) ?></div>
					<div
						class="tm-fonts__popup-code"><?php echo esc_html( '<i class="' ) . '<span class="tm-fonts__code"></span>' . esc_html( '"><!-- --></i>' ); ?></div>
					<div
						class="tm-fonts__popup-title"><?php esc_html_e( 'Shortcode', 'tm-helper-core' ) ?></div>
					<div class="tm-fonts__popup-code">[font-icon code="<span
							class="tm-fonts__code"></span>"
						size="" color=""]
					</div>
					<button type="button" class="dashicons dashicons-no-alt tm-fonts__popup-close"></button>
				</div>
			</div>
		</div>
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'tm-helper-core' ), $this->_version );
	} // End __clone ()
