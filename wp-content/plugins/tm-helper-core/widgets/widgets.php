...
require_once('widgets_files/footer-social-profile-widget.php');
require_once('widgets_files/recent_post_widget.php');
require_once('widgets_files/post_author_widget.php');
// Register Widgets
function tm_theme_helper_init_widgets() {
    // Footer Social
    register_widget( 'TM_THEME_HELPER_Social_Profiles' );
    // Footer Contact Info
    register_widget( 'FL_THEME_HELPER_Contact_Info' );
    // Blog Post
    register_widget('FL_THEME_HELPER_Recent_Post');
    if(class_exists('BuddyPress') && class_exists('Youzify')) {
        register_widget('FL_THEME_HELPER_Author_Box');
    }

}

add_action('widgets_init', 'tm_theme_helper_init_widgets');
if ( ! defined( 'ABSPATH' ) ) { exit; }
class TM_THEME_HELPER_WP_Widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        add_action('admin_enqueue_scripts', array($this, 'tm_register_scripts'),100);
        add_action('wp_enqueue_scripts', array($this,'tm_register_helper_scripts'));
    }

    function tm_register_scripts() {
        wp_enqueue_script('wp-color-picker');
        wp_enqueue_style('wp-color-picker');

    }

    function tm_register_helper_scripts() {
    ...
    protected function _input_number($name) {
        ...
        <p>
            <label for="<?php echo esc_attr($input_id); ?>"><?php echo esc_html($label); ?></label>
            <input type="number"  id="<?php echo esc_attr($input_id); ?>" name="<?php echo esc_attr($input_name); ?>" value="<?php echo esc_attr($value); ?>" />&nbsp;
            <?php echo wp_kses($desc_html, array('small' => array('id' => array(), 'style' => array()))); ?>
        </p>
        <?php
    }
    ...
    protected function _input_wp_dropdown_categories($name) {
        ...
        echo '<option value="">'.esc_html__('All categories' , 'tm-helper-core').'</option>';
    ...
    protected function _input_colorpicker($name) {
        ...
        echo '<p>';
        echo '<label for="'. esc_attr($input_id) .'">'.$label.'</label>';
        echo '<input type="text" class="tm-widget-colorpicker-field" id="'. esc_attr($input_id) .'" name="'.esc_attr($input_name).'" value="'.esc_attr($value).'" /></label>';
    ...
    protected function _input_upload_image($name) {
        ...
        <p>
            <label for="<?php echo esc_attr($input_id); ?>"><?php echo esc_html($label); ?></label>
            <img src="<?php echo (filter_var($value, FILTER_VALIDATE_URL)) ? esc_url( $value ) : ''; ?>" alt="" class="image_uploaded" style="<?php echo (filter_var($value, FILTER_VALIDATE_URL)) ? '' : 'display: none;'; ?> padding: 20px 0; max-width: 100px;display: block;" />
            ...
            <input class="upload_image_button button" type="button" value="<?php esc_html_e('Upload Image','tm-helper-core'); ?>" />
            <?php if(filter_var($value, FILTER_VALIDATE_URL)) : ?>
                <input class="remove_image_button button" type="button" value="<?php esc_html_e('Remove Image','tm-helper-core'); ?>" />
            <?php endif; ?>
